...
class HQCJNewsCrawler(NewsCrawler):
    ...
    def __init__(self, **params):
        ...
        self.list_pattern = re.compile(
            "<li><a target=\"_blank\" href=\"(http(.*?)html)\".*?>(.*?)<\/a><\/li>")
    ...
    def parse_list_page(self, content):
        ...
        for url,_,title in self.list_pattern.findall(content):
            ...
            if "html" in title:
                ...
                '\" title=\"((.|\n)*?)\">')
                page_title_data = page_title_pattern.search(title)
                news_info["title"] = page_title_data.group(1)
            else :

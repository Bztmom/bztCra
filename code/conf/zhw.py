...
pattern_conf = {
    "news": {
        "list_patterns": [
            r'<h3 class="item-tit"><a.*?href="(?P<url>.*?)">(?P<title>.*?)<\/a',
            r'<li><a.*?href="(?P<url>.*?)">(?P<title>.*?)<\/a'
        ],
        "news_patterns": [
            r'class="chan_scanToPhone">.*?<\/div>.*?<\/div>.*?<\/div>\s*</div>\s*(?P<pub_date>.*?)  (?P<source>.*?)<span.*?id="chan_newsDetail">(?P<content>.*?)<\/div'
        ]
    },
}
...
def default_generate_next_list_url(crawler):
    while crawler.page_num < 2:
        if crawler.page_num != 0:
            ret_url = crawler.list_url_tpl+"index_%d.html" % (crawler.page_num)
        else:
...
b_class_dict = {
    "滚动": {
        "url": "http://finance.china.com/roll/",
        "base_url": "http://finance.china.com",

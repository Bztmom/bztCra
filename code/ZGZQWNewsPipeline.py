...
class ZGZQWNewsContentParser(NewsProcessor):
    ...
    def process(self, news_info, doc):
        ...
        try:
            ...
            #     self.logger.warning("There is next page of news url: %s"%(news_info["url"]))
            ...
            try:
                ...
                self.logger.warning("Unable to parse date %s in format Y-m-d H:M:S from url:%s"%(date,news_info["url"]))
